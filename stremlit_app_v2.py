...
def save_reviews(review):
    # Convert your review data to a dataframe
    ...
    try:
        ...
        existing_data = conn.read(worksheet="Sheet2")
        
        # Print debugging information
        print("Existing data shape:", existing_data.shape)
        print("Existing data columns:", existing_data.columns.tolist())
        ...
        # Ensure columns match between dataframes
        ...
        updated_data = pd.concat([existing_data, review_df], ignore_index=True)
        print("Updated data shape:", updated_data.shape)
        
        # Update the Google Sheet with the combined data
        conn.update(data=updated_data, worksheet="Sheet2")
        
    except Exception as e:
        print("Error:", e)
        # If worksheet doesn't exist or is empty, just write the new data
        conn.update(data=review_df, worksheet="Sheet2")
...
def load_reviewed_cases_from_gsheets():
    try:
        # Read the data from Google Sheets
        df = conn.read(worksheet="Sheet2")
        
        # If the dataframe is empty or doesn't exist yet, return an empty set
        if df.empty:
            return set()
        
        # Extract the case names from the "case" column and return as a set
        return set(df["case"].tolist())
    except Exception as e:
        st.warning(f"Could not load reviewed cases: {e}")
        return set()
...
reviewed_cases = load_reviewed_cases_from_gsheets()
print("Reviewed cases:", reviewed_cases)
...
with col_center:
    ...
    with st.form("doctor_annotation_form", clear_on_submit=True):
        ...
        none_relevant = st.checkbox("None of the symptoms is relevant", key="rel_none")
            # If "None" is selected, all symptoms are irrelevant
            # Set relevant to an empty list and mark all as irrelevant
        ...
        if save_review:
            ...
            if all_fields_filled(relevant_resp, correct, comment, relevant, none_relevant):
                ...
                # Update the next case to display
                next_display = get_next_unreviewed_display_name(
                    display_names, 
                    load_reviewed_cases_from_gsheets().union({selected_case}), 
                    display_name_to_case, 
                    selected_display
                )
